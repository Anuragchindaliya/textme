...
import {
  createHPCharacterTableSQL,
  createWandTableSQL,
  dropHPCharacterTableSQL,
  dropWandTableSQL,
  insertHPCharacterSQL,
  insertWandSQL,
} from "./sql.js";
...
const main = async () => {
  ...
  const characterIndexToWandIndex: { [key: number]: number | null } = {};
  ...
  const getWandDataToSave = (data: any[]): any[] => {
    const wands = data
      .map((character, i) => {
      ...
      .filter((wand, i) => {
        ...
        if (isValidWand) {
          characterIndexToWandIndex[i] = Object.keys(
            characterIndexToWandIndex
          ).length;
        } else {
  ...
  const getCharacterDataToSave = (data: any[]) => {
    const formattedCharacters = data.map((character, i) => {
      ...
      retVal[10] =
        characterIndexToWandIndex[i] || characterIndexToWandIndex[i] === 0
          ? (characterIndexToWandIndex[i] as number) + 1
          : null;
      return retVal;
    });
    const characters = formattedCharacters.map((character) =>
