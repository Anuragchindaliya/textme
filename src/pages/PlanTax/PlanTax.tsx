...
const sections = {
  "80C": {
    label: "Section 80C (₹1,50,000 Limit)",
    limit: 150000,
    inputs: [
      {
        key: "ppf",
        label: "PPF (Public Provident Fund)",
        info: "PPF is a government-backed savings scheme with a 15-year lock-in, offering tax-free interest.",
      },
      {
        key: "elss",
        label: "ELSS (Equity Linked Saving Scheme)",
        info: "ELSS are tax-saving mutual funds with a 3-year lock-in period.",
      },
      {
        key: "lifeInsurance",
        label: "Life Insurance Premiums",
        info: "Premiums paid for life insurance policies qualify for tax deductions.",
      },
      {
        key: "nsc",
        label: "NSC (National Savings Certificate)",
        info: "NSC has a 5-year lock-in period, and interest is taxable.",
      },
      {
        key: "homeLoanPrincipal",
        label: "Home Loan Principal Repayment",
        info: "Principal repayment of a home loan is eligible under 80C.",
      },
      {
        key: "ssy",
        label: "Sukanya Samriddhi Yojana (SSY)",
        info: "SSY is a scheme for a girl child with tax-free returns.",
      },
      {
        key: "fd",
        label: "Fixed Deposits (5-year lock-in)",
        info: "Tax-saving fixed deposits have a 5-year lock-in.",
      },
    ],
  },
  "80D": {
    label: "Section 80D (₹25,000/₹50,000 Limit)",
    limit: 25000,
    inputs: [
      {
        key: "healthInsurance",
        label: "Health Insurance Premiums",
        info: "Premiums paid for health insurance qualify for deductions.",
      },
      {
        key: "preventiveHealthCheckup",
        label: "Preventive Health Checkup",
        info: "Preventive health checkups qualify up to ₹5,000 within the 80D limit.",
      },
    ],
  },
  ...
  // },
  "80E": {
    label: "Section 80E (No Limit)",
    limit: null,
    inputs: [
      {
        key: "educationLoanInterest",
        label: "Education Loan Interest",
        info: "Interest on education loans qualifies for deductions for 8 years.",
      },
    ],
  },
  "80CCD1B": {
    label: "Section 80CCD(1B) (₹50,000 Limit)",
    limit: 50000,
    inputs: [
      {
        key: "npsContribution",
        label: "NPS Contribution",
        info: "Additional NPS investment beyond the 80C limit.",
      },
    ],
  },
  "80TTA": {
    label: "Section 80TTA (₹10,000 Limit)",
    limit: 10000,
    inputs: [
      {
        key: "savingsAccountInterest",
        label: "Savings Account Interest",
        info: "Interest earned on savings accounts qualifies for deductions.",
      },
    ],
  },
  "80TTB": {
    label: "Section 80TTB (₹50,000 Limit)",
    limit: 50000,
    inputs: [
      {
        key: "seniorCitizenInterest",
        label: "Senior Citizen Interest",
        info: "Interest earned by senior citizens qualifies for deductions.",
      },
    ],
  },
  "24B": {
    label: "Section 24B (₹2,00,000 Limit)",
    limit: 200000,
    inputs: [
      {
        key: "homeLoanInterest",
        label: "Home Loan Interest",
        info: "Interest paid on home loans qualifies for deductions.",
      },
    ],
  },
  "80G": {
    label: "Section 80G (No Limit)",
    limit: null,
    inputs: [
      {
        key: "donations",
        label: "Donations",
        info: "Donations to charitable organizations qualify for deductions.",
      },
    ],
  },
  "80GGA": {
    label: "Section 80GGA (No Limit)",
    limit: null,
    inputs: [
      {
        key: "scientificResearchDonations",
        label: "Donations to Scientific Research",
        info: "Donations made for scientific research or rural development qualify for deductions.",
      },
    ],
  },
  "80U": {
    label: "Section 80U (₹75,000/₹1,25,000 Limit)",
    limit: 75000,
    inputs: [
      {
        key: "disability",
        label: "Disability Deduction",
        info: "Deductions for individuals with disabilities (higher limit for severe disability).",
      },
    ],
  },
  "80DDB": {
    label: "Section 80DDB (₹40,000/₹1,00,000 Limit)",
    limit: 40000,
    inputs: [
      {
        key: "medicalTreatment",
        label: "Medical Treatment",
        info: "Deductions for medical treatment of specified diseases.",
      },
    ],
  },
  "80EEA": {
    label: "Section 80EEA (₹1,50,000 Limit)",
    limit: 150000,
    inputs: [
      {
        key: "firstHomeLoanInterest",
        label: "First Home Loan Interest",
        info: "Additional deduction for first-time homebuyers on interest paid.",
      },
    ],
  },
  "80EEB": {
    label: "Section 80EEB (₹1,50,000 Limit)",
    limit: 150000,
    inputs: [
      {
        key: "evLoanInterest",
        label: "Electric Vehicle Loan Interest",
        info: "Interest paid on loans for electric vehicles qualifies for deductions.",
      },
    ],
  },
...
const TaxPlanner = () => {
  const {
    ...
    setValue,
    reset,
  } = useForm<z.infer<typeof taxSchema>>({
  ...
  const [totalDeductionNum, setTotalDeductionNum] = useState<number | null>(
    null,
  )
  ...
  const onSubmit = (data: z.infer<typeof taxSchema>) => {
    ...
    Object.values(sections).forEach((section) => {
      section.inputs.forEach(({ key }) => {
        let value = Number(
          data[key as keyof typeof data]?.replace(/,/g, "") || 0,
        )
        if (!section.limit) {
      ...
      <Card className="p-6 container mx-auto mt-10">
        <CardContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {totalDeductionNum !== null && (
              <p className="mt-4 text-lg font-bold">
                Deduction: ₹{totalDeductionNum.toLocaleString("en-IN")}
              </p>
            )}
            {taxableIncomeRs !== null && (
              <p className="mt-4 text-lg font-bold">
                Taxable Income : ₹{taxableIncomeRs.toLocaleString("en-IN")}
              </p>
            )}
            {calculatedTax !== null && (
              <p className="mt-4 text-lg font-bold">
                Estimated Tax: ₹{calculatedTax.toLocaleString("en-IN")}
              </p>
            )}
          </form>
            ...
            <div className="border-b">
              <CardHeader>
                ...
                <p className="text-sm text-gray-500">
                  Based on <span className="capitalize">{watch("regime")}</span>{" "}
                  Tax Regime
                </p>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="text-right font-bold">
                    ₹{formatCurrency(totalDeductionNum?.toString())}
                  </div>
                  <div className="font-semibold">Taxable Income</div>
                  <div className="text-right font-bold">
                    ₹{formatCurrency(taxableIncomeRs?.toString())}
                  </div>
                  <div className="font-semibold">Income Tax</div>
                  <div className="text-right font-bold">
                    ₹{formatCurrency(calculatedTax?.toString())}
                  </div>
                  <div className="font-semibold">
                    Health & Education Cess (4%)
                  </div>
                  <div className="text-right font-bold">
                    ₹{formatCurrency(totalCess?.toString())}
                  </div>
            ...
            <div className="mt-6">
              ...
              <CardContent>
                <Collapsible className="border border-gray-200 bg-gray-100 text-gray-800 rouded-lg text-sm">
                  <CollapsibleTrigger className="font-semibold  w-full text-left p-2 ">
                    The New Tax Regime is more beneficial!
                  </CollapsibleTrigger>
                ...
                <ul className="mt-4 space-y-2 list-disc pl-5 text-sm text-gray-700">
                  <li>
                    Maximize your Section 80C investments (PPF, ELSS, NPS) up to
                    ₹1,50,000 to save ₹18,000.
                  </li>
                  <li>
                    Consider health insurance premiums under Section 80D (up to
                    ₹25,000) to save ₹18,000.
                  </li>
                  <li>
                    This is an estimate. Consult a tax professional for detailed
                    advice.
                  </li>
                </ul>
