...
const initialNodes: Node[] = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || "[]");
const initialEdges: Edge[] = JSON.parse(localStorage.getItem(LOCAL_STORAGE_EDGES_KEY) || "[]");
...
export default function FlowBuilder() {
  ...
  const onConnect = (params: Edge | Connection) => setEdges((eds) => addEdge({
    ...params,
    markerEnd: {
  ...
  const syncToSheetDB = async () => {
    const res = await fetch("https://sheetdb.io/api/v1/YOUR_SHEETDB_ID", {
      ...
      body: JSON.stringify({ data: [{ nodes: JSON.stringify(nodes), edges: JSON.stringify(edges) }] }),
    });
    ...
    <div className="flex flex-col md:flex-row w-full h-screen">
      <div className="w-full md:w-1/4 p-4 border-r space-y-4 overflow-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
          <input {...register("label")} className="border p-2 rounded w-full" placeholder="Node Label" />
        ...
        <Button onClick={exportToJSON} className="w-full">Export to JSON</Button>
        <input type="file" accept=".json" onChange={importFromJSON} ref={fileInputRef} className="hidden" />
        <Button onClick={() => fileInputRef.current?.click()} className="w-full">Import from JSON</Button>
        <Button onClick={syncToSheetDB} className="w-full bg-green-500 hover:bg-green-600">Sync to SheetDB</Button>
        <Button onClick={exportToImage} className="w-full">Export as Image</Button>
        <Button onClick={exportToPDF} className="w-full">Export as PDF</Button>
          ...
          <div className="p-10 space-y-4">
            {nodes.map((node) => (
              <div key={node.id} className="border p-4 rounded">
                <label className="block font-semibold mb-1">{node.data.label}</label>
                {node.data.type === "input" && <input className="border p-2 w-full rounded" placeholder="Type here..." />}
                ...
                {node.data.type === "text" && <textarea className="border p-2 w-full rounded" rows={3} />}
              </div>
            ))}
          </div>
        )}
