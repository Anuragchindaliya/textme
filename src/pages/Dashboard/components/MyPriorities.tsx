...
import { useGetAssignedTasksQuery, useUpdateTaskMutation } from "@/features/task/tasksApi"
        ...
        <ScrollArea className="h-[350px] pr-4">
          <div className="space-y-4">
            {tasks?.map((task) => (
              <div
                key={task.id}
                className={`flex items-center justify-between rounded-lg border p-3 shadow-sm transition-all hover:bg-accent ${task.status === 'done' ? 'opacity-50' : ''}`}
              >
                <div className="flex flex-col space-y-1">
                    ...
                    <span className="text-xs text-muted-foreground ml-8">{task.id}</span>
                </div>
                <div className="flex items-center space-x-2">
            ...
            {(!tasks || tasks.length === 0) && <div className="text-center text-muted-foreground">No tasks assigned.</div>}
          </div>
        </ScrollArea>
      </CardContent>
    </Card>
